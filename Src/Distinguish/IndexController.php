...
class IndexController
{
     ...
     * @return string
     */
    public function getResult( $path = null , $returnStyle = 'autoStudyResult' )
    {
        $imageArr       = $this->getImageInfoProvider->getResult( $path );
        $imageInfo      = $imageArr['imageInfo'];
        $imageBinaryArr = $imageArr['imageBinaryArr'];

        $noiseCancelArr = $this->pretreatmentProvider->run(  $imageInfo , $imageBinaryArr );
        $charArr        = $this->cuttingProvider->run( $noiseCancelArr , $imageInfo );
        $result         = $this->identifyProvider->run( $charArr , $returnStyle );
